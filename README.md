...
2. В корневой директории репозитория исполнить команду:
   ...
   `.RDS`, а также результаты для каждого региона в каталоге `Results/`. Расчёт использует параллельные вычисления (по умолчанию 16 рабочих процессов, конфигурируется в `R/_targets.R`) и является ресурсоёмким. Полноценная репликация может занимать от нескольких дней до недели, в зависимости от мощности компьютера.

3. После завершения расчётов рукопись статьи и приложения могут быть реплицированы с помощью команд:
